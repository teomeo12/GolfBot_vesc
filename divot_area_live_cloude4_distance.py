...
def main():
    ...
    # Load YOLO Model
    try:
        # The script is in the workspace root, so we build the path to the model inside the package.
        script_dir = os.path.dirname(os.path.abspath(__file__))
        model_path = os.path.join(script_dir, 'src', 'differential_drive', '1600s_aug_100ep.pt')

        if not os.path.exists(model_path):
            # Provide a clear error message if the model is not found at the expected path.
            raise FileNotFoundError(f"Model file not found. Expected it at: {model_path}")
