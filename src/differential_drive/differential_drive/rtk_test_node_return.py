...
class RTKTestNodeReturn(Node):
    def __init__(self):
        ...
        self.state = "IDLE"  # IDLE, DRIVING_FORWARD, TURNING, DRIVING_BACK
        self.prev_a_state = False
        
        # Parameters
        self.drive_speed = 550.0
        self.heading_kp = 2.5
        self.turn_tolerance_rad = math.radians(5) # 5 degrees
        self.distance_tolerance_m = 0.1 # 10 cm

        # Control loop timer
        self.create_timer(0.1, self.control_loop)
        
        self.get_logger().info('RTK Test Node (Return Trip) Initialized')
        self.get_logger().info('Press A Button: Start 1m There-and-Back Test')
    ...
    def start_rtk_test(self):
        ...
        self.results_pub.publish(String(data=f"START | Lat: {self.start_pos[0]:.8f}, Lon: {self.start_pos[1]:.8f}"))
    ...
    def control_loop(self):
        if self.state == "IDLE" or self.current_lat is None or self.current_yaw is None: return
        ...
        if self.state == "DRIVING_FORWARD":
            ...
            if distance_traveled < 1.0:
                self.drive_straight()
                self.publish_status(f"State: FWD | Dist: {distance_traveled:.2f}m")
            else:
                self.get_logger().info("✅ Reached 1m mark. State -> TURNING")
                self.state = "TURNING"
        ...
        # --- State: Turning ---
            ...
            if abs(heading_error) > self.turn_tolerance_rad:
                cmd = Twist()
                cmd.angular.z = self.heading_kp * heading_error * 2 # Turn faster
        ...
        # --- State: Driving Back ---
            ...
            distance_to_start = self.gps_to_meters(self.start_pos[0], self.start_pos[1], self.current_lat, self.current_lon)
            if distance_to_start > self.distance_tolerance_m:
                self.drive_straight()
                self.publish_status(f"State: BACK | Dist to Start: {distance_to_start:.2f}m")
            else:
                self.get_logger().info("🏁 TEST COMPLETE 🏁")
                self.state = "IDLE"
                self.send_stop_command()
                final_pos = (self.current_lat, self.current_lon)
                closing_error = self.gps_to_meters(self.start_pos[0], self.start_pos[1], final_pos[0], final_pos[1])
                self.get_logger().info(f"Final Closing Error: {closing_error*100:.1f} cm")
                self.results_pub.publish(String(data=f"END | Lat: {final_pos[0]:.8f}, Lon: {final_pos[1]:.8f} | Closing Error: {closing_error:.3f}m"))
